...
const generalRateLimit = rateLimit({
  ...
  limit: 100 * maxMultiple, // Limit each IP to 100 requests per `window` in prod, 10,000 in testing (per the winowMs time).
...
app.use((req, res, next) => {
  ...
  if (req.method !== 'GET' && req.method !== 'HEAD') {
    if (
      req.method !== 'GET' &&
      protectedPaths.some((path) => req.path.includes(path))
    ) {
