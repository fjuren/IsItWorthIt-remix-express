...
import { ActionFunctionArgs, LoaderFunctionArgs, MetaFunction, useFetcher, useLocation } from 'react-router';
...
async function fetchGameCheapShark(gameTitle: string, pageNum: number, pageSize: number): Promise<any> {
  const requestOptions = {
    method: 'GET',
  };
...
export async function loader( {request}: LoaderFunctionArgs ) {
  ...
  if (url.search) {
    ...
    console.log(url)
    gameDeals = await fetchGameCheapShark(gameTitleSearch, INITIAL_PAGE, PAGE_SIZE)
  } else {
  ...
  const stores = await fetchStoresCheapShark()
    ...
    return {initialGames: gameDeals, searchGameTitle: gameDeals, stores, hasMore: gameDeals.length === PAGE_SIZE}
  }
...
export default function HomeRoute() {
  const { initialGames, stores, hasMore: initHasMore} = useLoaderData<typeof loader>();
    ...
    <>
      ...
      <div>
        <Form method='GET' {...getFormProps(form)} className='flex gap-2'>
          <InputWithIcon startIcon={Search} name='gameTitle' type='search' placeholder='Search game title'/>
      ...
      gameDeals.map((game: Deal, index: number) => {
          ...
          return (
            <GameCard key={index} gameId={game.gameID} ref={index === gameDeals.length - 1 ? lastGameRef : null}>
            <GameCardImage
              src={game.thumb}
